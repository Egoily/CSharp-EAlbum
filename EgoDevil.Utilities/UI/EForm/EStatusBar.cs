...
namespace EgoDevil.Utilities.UI.EForm
{
    ...
    {
        public enum EStatusBarBackImageAlign
        {
            /// <summary>
            /// Left aligned image.
            /// </summary>
        ...
        private Color m_clrBarBorder = Color.FromArgb(191, 213, 237);
        private List<EBarItem> m_xbItems = new List<EBarItem>();
        ...
        private void DrawStatusGlow(Rectangle rect, Graphics g, Color clrGlowColor)
        {
        ...
        private void DrawStatusBarBackImage(Graphics g, Rectangle rcStatusbar)
        {
            if (m_BarBackground != null)
            {
                ...
                {
                    case EStatusBarBackImageAlign.Right:
        ...
        public class EBarItem
        {
            ...
            public EBarItem(int lWidth)
            {
                m_lWidth = lWidth;
            }

            public EBarItem(int lWidth, string sText)
            {
                m_lWidth = lWidth;
                m_sItemText = sText;
            }
            ...
            public void RenderXBarItem(Graphics g, int lLeft, int lTop)
            {
                ...
                // draw separators:
                g.DrawLine(pSeparator, rcItem.Left + rcItem.Width, rcItem.Top + 1, rcItem.Left + rcItem.Width, rcItem.Top + m_lHeight - 1);
                pSeparator = new Pen(m_clrSeparatorInner);
                g.DrawLine(pSeparator, rcItem.Left + rcItem.Width + 1, rcItem.Top + 1, rcItem.Left + rcItem.Width + 1, rcItem.Top + m_lHeight - 1);
                ...
                g.DrawString(m_sItemText, m_fntItemFont, sb, rcText, sf);
        ...
        {
            private Color m_clrForeRectStart = Color.FromArgb(118, 151, 202);
            private Color m_clrForeRectEnd = Color.FromArgb(169, 203, 248);
            private Color m_clrBackRect = Color.FromArgb(184, 206, 235);
            ...
            {
                Rectangle rcForeRect = new Rectangle(lX, lY, 2, 2);
                Rectangle rcBackRect = new Rectangle(lX + 1, lY + 1, 2, 2);

                SolidBrush sbForeStart = new SolidBrush(m_clrForeRectStart);
                SolidBrush sbBack = new SolidBrush(m_clrBackRect);

                g.FillRectangle(sbBack, rcBackRect);

                g.FillRectangle(sbForeStart, rcForeRect);
