...
namespace EgoDevil.Utilities.UI.Docking
{
	public class DockContent : Form, IDockContent
	{
		public DockContent()
		{
            ...
            m_dockHandler.DockHided += new EventHandler(DockHandler_DockHided);
            m_dockHandler.DockClosing += new EventHandler(DockHandler_DockClosing);
        ...
        private void DockHandler_DockHided(object sender, EventArgs e)
        {
            OnDockHided(e);
        }

        private void DockHandler_DockClosing(object sender, EventArgs e)
        {
            OnDockClosing(e);
        }
        ...
        //-----------------
        private static readonly object DockHidedEvent = new object();
        [LocalizedCategory("Category_PropertyChanged")]
        [LocalizedDescription("Pane_DockHided_Description")]
        public event EventHandler DockHided
        {
            add { Events.AddHandler(DockHidedEvent, value); }
            remove { Events.RemoveHandler(DockHidedEvent, value); }
        }
        protected virtual void OnDockHided(EventArgs e)
        {
            EventHandler handler = (EventHandler)Events[DockHidedEvent];
            if (handler != null)
                handler(this, e);
        }
        ...
        private static readonly object DockClosingEvent = new object();
        [LocalizedCategory("Category_PropertyChanged")]
        [LocalizedDescription("Pane_DockClosing_Description")]
        public event EventHandler DockClosing
        {
            add { Events.AddHandler(DockClosingEvent, value); }
            remove { Events.RemoveHandler(DockClosingEvent, value); }
        }
        protected virtual void OnDockClosing(EventArgs e)
        {
            EventHandler handler = (EventHandler)Events[DockClosingEvent];
            if (handler != null)
                handler(this, e);
        }
