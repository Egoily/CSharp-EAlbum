    ...
    internal class VS2005DockPaneStrip : DockPaneStripBase
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (Appearance == DockPane.AppearanceStyle.Document)
            {
                ...
                // Add these values back in so that the DockStrip color is drawn beneath the close
                // button and window list button.
                ...
                // It is possible depending on the DockPanel DocumentStyle to have a Document
                // without a DockStrip.
        ...
        private void CalculateTabs_Document()
        {
            ...
            // Originally all new documents that were considered overflow (not enough pane strip
            // space to show all tabs) were added to the far left (assuming not right to left) and
            // the tabs on the right were dropped from view. If StartDisplayingTab is not 0 then we
            // are dealing with making sure a specific tab is kept in focus.
            if (m_startDisplayingTab > 0)
            {
                ...
                // Store which tab is the first one displayed so that it will be drawn correctly
                // (without part of the tab cut off)
                FirstDisplayingTab = EndDisplayingTab;

                tempX = x; // Reset X location because we are starting over

                // Start with the first tab displayed - name is a little misleading. Loop through
                // each tab and set its location. If there is not enough room for all of them
        ...
        protected override void OnLayout(LayoutEventArgs levent)
        {
            ...
            // If the close button is not visible draw the window list button overtop. Otherwise it
            // is drawn to the left of the close button.
